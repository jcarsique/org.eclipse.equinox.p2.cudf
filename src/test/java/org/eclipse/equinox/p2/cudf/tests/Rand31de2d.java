...
public class Rand31de2d extends TestCase {
    public void testLibdmx1() {
        ProfileChangeRequest pcr = new Parser().parse(new File(
                Rand31de2d.class.getClassLoader().getResource(
                        "testData/instances/expectedSuccess/rand31de2d-sol.cudf").toString().substring(
                        "file:".length())));
        pcr.addInstallableUnit(new RequiredCapability("libtext-wrapi18n-perl",
        ...
        restart: for (Iterator iterator = res.iterator(); iterator.hasNext();) {
            ...
            pcr2.addInstallableUnit(new RequiredCapability(iu.getId(),
                    new VersionRange(iu.getVersion())));
    ...
    private QueryableArray slice(QueryableArray input, String id,
            Version version) {
        return new Slicer(input).slice(
                (InstallableUnit) input.query(
                        new CapabilityQuery(new RequiredCapability(id,
                                new VersionRange(version))), new Collector(),
                        null).iterator().next(), null);
    }
