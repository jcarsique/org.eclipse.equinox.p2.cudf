 ...
 * all the inner maps.
 */
public class TwoTierMap implements Map<Object, Object>, Serializable {
    ...
    public static final int POLICY_NONE = 0,
            POLICY_BOTH_MAPS_PRESERVE_ORDERING = 1 << 0,
            POLICY_INNER_MAP_PRESERVE_EXISTING = 1 << 1,
            POLICY_INNER_MAP_SORTED_ASCENDING = 1 << 2,
            POLICY_INNER_MAP_SORTED_DESCENDING = 1 << 3,
            POLICY_INNER_MAP_ENSURE_SINGLETON = 1 << 4;

    private static final int POLICY_INNER_MAP_SORTED_MASK = POLICY_INNER_MAP_SORTED_ASCENDING
    ...
    public TwoTierMap(int initialCapacity, int policy) {
        this.policy = policy;
        this.outerMap = shouldUseOrderedMap() ? new LinkedHashMap<Object, Map<Object, Object>>(
                initialCapacity) : new HashMap<Object, Map<Object, Object>>(
                initialCapacity);
    }
    ...
    public Collection<?> getAll(Object key1) {
        if (key1 == null)
            ...
            return values();
        Map<Object, Object> innerMap = this.outerMap.get(key1);
        return innerMap == null ? Collections.EMPTY_LIST
                : Collections.unmodifiableCollection(innerMap.values());
    ...
    public boolean containsValue(Object value) {
        ...
        Iterator<Entry<?, Map<?, ?>>> it = entrySet().iterator(); it.hasNext();) {
    ...
    public Set entrySet() {
        Set<Entry<Object, Map<Object, Object>>> unmodifiableSet = Collections.unmodifiableSet(this.outerMap.entrySet());
    ...
    public Collection<Object> values() {
        ...
        for (Iterator<Object> it = this.outerMap.keySet().iterator(); it.hasNext();) {
            Object key = it.next();
    ...
    public String toString() {
        ...
        if (this.outerMap.isEmpty()) {
            ...
            for (Iterator<Entry<Object, Map<Object, Object>>> it = this.outerMap.entrySet().iterator(); it.hasNext();) {
                Entry<Object, Map<Object, Object>> entry = it.next();
    ...
    private Map<Object, Object> createInnerMap() {
        ...
        if (shouldUseSortedInnerMap()) {
            innerMap = new TreeMap<Object, Object>(new ValueComparator(
                    shouldSortInAscendingOrder()));
        } else if (shouldUseOrderedMap()) {
            innerMap = new LinkedHashMap<Object, Object>(2);
        } else {
            innerMap = new HashMap<Object, Object>(2);
        }
        return innerMap;
    }
