...
public class Parser {
    ...
    private static final boolean FORCE_QUERY = true; // TO SET TO FALSE FOR
                                                     // COMPETITION

    private static final boolean DEBUG = false; // TO SET TO FALSE FOR
                                                // COMPETITION

    private static final boolean TIMING = true; // TO SET TO FALSE FOR
                                                // COMPETITION
    ...
    public ProfileChangeRequest parse(InputStream stream,
            boolean includeRecommends, String sumProperty) {
        long start;
        if (TIMING)
        ...
        try {
            ...
            while (true) {
                ...
                for (next = reader.readLine(); next != null
                        && next.length() > 1 && next.charAt(0) == ' '; next = reader.readLine()) {
                ...
                // preamble stanza
                if (line.startsWith("#") || line.startsWith("preamble: ")
                        || line.startsWith("property: ")
                        || line.startsWith("univ-checksum: ")) {
                    // ignore
                }
                ...
                // package stanza
                    ...
                    handleKeep(line);
                } else if (sumProperty != null
                        && line.startsWith(sumProperty + ":")) {
    ...
    private void handleKeep(String line) {
        ...
        if (line.contains("version")) {
            currentKeepRequests.add(new RequiredCapability(currentIU.getId(),
                    new VersionRange(currentIU.getVersion()), false));
            return;
        }
        ...
        if (line.contains("feature")) {
            ...
            for (int i = 0; i < caps.length; i++) {
                if (!caps[i].getName().equals(currentIU.getId()))
                    currentKeepRequests.add(new RequiredCapability(
                            caps[i].getName(), caps[i].getVersion(), false));
            }
        }
    ...
    private void handleExpected(String line) {
        currentRequest.setExpected(Integer.decode(
                line.substring("expected: ".length()).trim()).intValue());
    }
        ...
        if (currentIU.getVersion() == null)
            throw new IllegalStateException(
                    "Malformed \'package\' stanza. Package "
                            + currentIU.getId() + " does not have a version.");
        if (currentIU.getProvidedCapabilities().length == 0) {
            currentIU.setCapabilities(new IProvidedCapability[] { new ProvidedCapability(
                    currentIU.getId(), new VersionRange(currentIU.getVersion(),
                            true, currentIU.getVersion(), true)) });
        }
    ...
    private void handleInstalled(String line) {
        ...
        currentIU.setInstalled(Boolean.valueOf(value));
        if (currentIU.isInstalled()) {
            preInstalled.add(new RequiredCapability(currentIU.getId(),
    ...
    private void handleUpgrade(String line) {
        ...
        for (Iterator<IRequiredCapability> iterator = updateRequest.iterator(); iterator.hasNext();) {
            ...
            currentRequest.upgradeInstallableUnit(new RequiredCapability(
                    requirement.getName(), VersionRange.emptyRange, 1));

            // Add a requirement forcing the solution to be greater or equal to
    ...
    private IRequiredCapability getHighestInstalledVersion(
        ...
        for (Iterator<?> iterator = c.iterator(); iterator.hasNext();) {
            ...
            if (candidate.getId().equals(req.getName())) {
                if (highestVersion == null
                        || candidate.getVersion().getMajor() > highestVersion.getMajor())
                ...
                for (int i = 0; i < prov.length; i++) {
                    if (prov[i].getVersion().equals(VersionRange.emptyRange))
                        continue;
                    if (prov[i].getName().equals(req.getName())
                            && (highestVersion == null || prov[i].getVersion().getMinimum().getMajor() > highestVersion.getMajor()))
        ...
        if (highestVersion == null)
            return null;
        return new RequiredCapability(req.getName(), new VersionRange(
                highestVersion, true, Version.maxVersion, true));
    }
    ...
    private void handleVersion(String line) {
        currentIU.setVersion(new Version(
                cudfPosintToInt(line.substring("version: ".length()))));
    }
    ...
    private void handleRecommends(String line) {
        mergeRequirements(createRequires(
                line.substring("recommends: ".length()), true, true, true));
    }
     ...
     * If there is more than one entry for a particular package, the extra
    ...
    private List<IRequiredCapability> createRequires(String line,
        ...
        while (s.hasMoreElements()) {
            StringTokenizer subTokenizer = new StringTokenizer(s.nextToken(),
                    "|");
            if (subTokenizer.countTokens() == 1) { // This token does not
                                                   // contain a |.
                ...
                if ("true!".equals(subtoken)) {
                    ...
                    throw new RuntimeException(
                            "Cannot have true! in a conflict!!!!!");
                }
                if ("false!".equals(subtoken)) {
                    if (!dependency)
                        continue;
                    throw new RuntimeException(
                            "Cannot have false! in a dependency!!!!!");
                }
            ...
            while (subTokenizer.hasMoreElements()) {
                ors[i++] = (IRequiredCapability) createRequire(
                        subTokenizer.nextToken(), expandNotEquals, optional);
            }
    ...
    private Object createRequire(String nextToken, boolean expandNotEquals,
        ...
        if (tokenCount == 1) // a
            return new RequiredCapability(expressionTokens.nextToken().trim(),
                    VersionRange.emptyRange, optional);

        if (tokenCount == 3) // a > 2, a < 2, a = 2
            return new RequiredCapability(expressionTokens.nextToken().trim(),
                    createRange3(expressionTokens.nextToken(),
                            expressionTokens.nextToken()), optional);
        ...
        if (tokenCount == 4) { // a >= 2, a <=2, a != 2
            ...
            if (!("!".equals(signFirstChar))) // a >= 2 a <= 2
                return new RequiredCapability(id, createRange4(signFirstChar,
                        version), optional);

            // a != 2
            if (expandNotEquals) {
                return new ORRequirement(new IRequiredCapability[] {
    ...
    private VersionRange createRange3(String sign, String versionAsString) {
        ...
        sign = sign.trim();
        if (">".equals(sign))
            return new VersionRange(new Version(version), false,
                    Version.maxVersion, false);
        if ("<".equals(sign))
            return new VersionRange(Version.emptyVersion, false, new Version(
                    version), false);
        if ("=".equals(sign))
    ...
    private VersionRange createRange4(String sign, String versionAsString) {
        ...
        if (">".equals(sign)) // THIS IS FOR >=
            return new VersionRange(new Version(version), true,
                    Version.maxVersion, false);
        if ("<".equals(sign)) // THIS IS FOR <=
            return new VersionRange(Version.emptyVersion, false, new Version(
                    version), true);
    ...
    private IProvidedCapability createProvidedCapability(Tuple tuple) {
        ...
        if (extraData == null)
            return new ProvidedCapability(tuple.name, createVersionRange(
                    tuple.operator, tuple.version));
        // 2 constraints (e.g. a>=1, a<4) so create a real range like a[1,4)
        if (extraData.size() == 1)
            return new ProvidedCapability(tuple.name, createVersionRange(tuple,
                    (Tuple) extraData.iterator().next()));
    ...
    /*
     * Create and return a version range object which merges the 2 given
    ...
    private VersionRange createVersionRange(Tuple t1, Tuple t2) {
        ...
        if (one.compareTo(two) < 0) {
            return new VersionRange(one, include(t1.operator), two,
                    include(t2.operator));
        } else if (one.compareTo(two) == 0) {
            return new VersionRange(one, include(t1.operator), one,
                    include(t1.operator));
        } else if (one.compareTo(two) > 0) {
            return new VersionRange(two, include(t2.operator), one,
                    include(t1.operator));
        }
    ...
    /*
     * Helper method for when we are creating version ranges and calculating
     * "includeMin/Max".
     */
    ...
    /*
     * Create and return a version range based on the given operator and number.
     * Note that != is
     * handled elsewhere.
     */
    ...
    private void debug(ProfileChangeRequest request) {
        ...
        // Log.println("Adding IU: " + toAdd[i].getId() + ' ' +
        // toAdd[i].getVersion());
        // }
        ...
        // Log.println("Adding IU property: " + key + "->" +
        // propsToAdd.get(key));
        // }
        // }
        //
        ...
        // Log.println("Removing IU: " + toRemove[i].getId() + ' ' +
        // toRemove[i].getVersion());
        // }
        // Map propsToRemove =
        // request.getInstallableUnitProfilePropertiesToRemove();
        // if (propsToRemove == null || propsToRemove.isEmpty()) {
        // Log.println("No IU properties to remove.");
        // } else {
        // for (Iterator iter = propsToRemove.keySet().iterator();
        // iter.hasNext();) {
        // Object key = iter.next();
        // Log.println("Removing IU property: " + key + "->" +
        // propsToRemove.get(key));
        // }
        // }
    }
