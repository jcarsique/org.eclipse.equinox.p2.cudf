...
public class Slicer {
    private static boolean TIMING = false; // SET THIS TO FALSE FOR THE
                                           // COMPETITION

    private QueryableArray possibilites;

    private LinkedList<InstallableUnit> toProcess;

    private Set<InstallableUnit> considered; // IUs to add to the slice
    ...
    private void handleExtraRequirements(
            List<IRequiredCapability> extraRequirements) {
        if (extraRequirements != null)
            for (Iterator<IRequiredCapability> iterator = extraRequirements.iterator(); iterator.hasNext();) {
                expandRequirement(null, iterator.next());
            }
    }
    ...
    public QueryableArray slice(InstallableUnit ius,
        ...
        try {
            ...
            considered = new HashSet<InstallableUnit>(possibilites.getSize());
            considered.add(ius);
            toProcess = new LinkedList<InstallableUnit>(considered);
        ...
        return new QueryableArray(
                considered.toArray(new InstallableUnit[considered.size()]));
    }
    ...
    private void addHighestVersion(InstallableUnit iu) {
        Collector matches = possibilites.query(
                new CapabilityQuery(new RequiredCapability(iu.getId(),
                        VersionRange.emptyRange, 1)), new Collector(), null);
        if (matches.size() == 1 || matches.size() == 0)
            return;

        InstallableUnit highestVersion = iu;
        for (@SuppressWarnings("unchecked")
        Iterator<InstallableUnit> iterator = (Iterator<InstallableUnit>) matches.iterator(); iterator.hasNext();) {
            InstallableUnit candidate = iterator.next();
    ...
    private void expandRequirement(InstallableUnit iu, IRequiredCapability req) {
        if (req.isNegation())
            return;
        Collector matches = possibilites.query(new CapabilityQuery(req),
                new Collector(), null);
        int validMatches = 0;
        for (@SuppressWarnings("unchecked")
        Iterator<InstallableUnit> iterator = (Iterator<InstallableUnit>) matches.iterator(); iterator.hasNext();) {
            InstallableUnit match = iterator.next();
            validMatches++;
        ...
        if (validMatches == 0) {
            if (req.isOptional()) {
                ...
                result.add(new Status(IStatus.WARNING, Main.PLUGIN_ID,
                        NLS.bind(Messages.Planner_Unsatisfied_dependency, iu,
                                req)));
            }
        }
    }
