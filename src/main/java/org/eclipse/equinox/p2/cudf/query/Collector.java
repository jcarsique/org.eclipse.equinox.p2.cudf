...
public class Collector implements IQueryable {
     ...
     * override this method to perform additional filtering, add different
     * objects to the list, short-circuit the traversal, or process the objects
     * directly without collecting them.
     * 
     ...
     * {@linkplain #toCollection()},
     * this returns the actual modifiable collection that is being used to store
     * results. The
     * return value is only intended to be used within subclasses and should not
     * be exposed
    ...
    public Iterator<?> iterator() {
        return collected == null ? Collections.EMPTY_LIST.iterator()
                : collected.iterator();
    }
    ...
    public Collection<?> toCollection() {
        return collected == null ? Collections.EMPTY_SET
                : Collections.unmodifiableSet(collected);
    }
    ...
    public Collector query(Query query, Collector collector,
        ...
        Iterator<? extends Object> iter = collector == this ? toCollection().iterator()
                : iterator();
        if (monitor == null)
